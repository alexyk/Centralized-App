...
class AirTicketsSearchFilterPanel extends Component {
  constructor(props) {
    ...
    this.requestFilters = this.requestFilters.bind(this);
  }

  requestFilters() {
    this.props.requestFilters().then(filters => {
      this.setState({
        filters: filters
      });
    });
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.allowFiltering !== this.props.allowFiltering) {
      this.requestFilters();
    }
  }
  ...
  render() {
    console.log("Wazaaa");
    const { loading, windowWidth, showFiltersMobile } = this.props;
    const {
      priceRange,
      filters,
      waitingTimeRange,
