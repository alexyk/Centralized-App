...
function filterFlights(filters, flights) {
  ...
  if (filters.waiting) {
    flights = filterByWaitingTime(filters, flights);
  }
...
function _isDirect(flight) {
  let segmentsByGroup = _.groupBy(_.prop("group"), flight.orderedSegments);
...
describe("flight filtering", () => {
  test("it works", () => {
    let result = filterFlights(filters, flights);
