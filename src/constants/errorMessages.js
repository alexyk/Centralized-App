export const NOT_FOUND = "404, Page not found.";
export const PROFILE_UPDATE_ERROR =
  "There was an error updating your user profile. Please try again.";
export const PROPERTY_CANNOT_BE_DELETED =
  "Cannot delete this property. It might have active reservations or other pending actions.";
export const SOCIAL_PROFILE_EMPTY = "Social profile field cannot be empty.";
export const INVALID_URL = "Please enter a valid URL.";
export const SERVICE_UNAVAILABLE = "Service unavailable.";
export const ROOM_IS_NO_LONGER_AVAILABLE =
  "This room is no longer available. Please try another one or try again later.";
export const USER_NOUSER = "User NOT FOUND";
export const USER_SERVERROR = "Backend Error";

export const MISSING_ORIGIN = "Please enter origin city.";
export const MISSING_DESTINATION = "Please enter destination city.";
export const MISSING_CHILDREN_AGE = "Please specify children age between 1 and 12.";
export const MORE_INFANTS_THEN_ADULTS = "More infants then adults";

export const NOT_ADDED_TO_BLACKLIST = "not added to blacklist.";
export const NOT_REMOVED_FROM_BLACKLIST = "not removed from blacklist.";
